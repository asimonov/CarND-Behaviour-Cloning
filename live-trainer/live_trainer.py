...
class LiveTrainer(object):
    ...
    def start_server(self):
        print('control_srv.start()')
        self.control_srv.start() # Start server
        print('control_srv started')
    ...
    def preprocess_input(self, img):
        print('resize')
        im2 = sp.imresize(img, size=shape, interp='cubic')
        print('mask ',im2.shape,' ',mask.shape)
        im2 = np.multiply(im2,mask).astype(np.uint8)
        print('preprecess: ',im2.shape)
        return im2
    ...
    def handle_telemetry(self, data):
        ...
        print('handle_telemetry')
        if self.mode == 'auto':
            print('predicting on telemetery')
            self.steering_angle = self.predict_steering(data)
            print('predicted angle = ',self.steering_angle)
        elif self.mode == 'manual':
            print('manual on telemetery')
